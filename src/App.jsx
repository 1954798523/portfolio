...
const PROJECTS = [
  {
    ...
    color: "#a78bfa",
    metrics: [
      { big: "4 个", lbl: "AI 服务 7×24 运行" },
      { big: "RTX 5090", lbl: "自建 GPU 工作站" },
      { big: "90%+", lbl: "生图可用率" },
    ],
    ...
    solution: [
      "采购并装机 RTX 5090D 工作站，从裸机开始配置 CUDA 环境，部署 ComfyUI + FLUX.1-dev 推理管线",
      "提炼 Prompt 工程方法论：5 条规则 + 禁用词表 + 姿势字典，封装为 HTML 交互界面，员工输入中文需求自动输出专业 prompt",
      "搭建统一入口（8080 端口）集成工具箱，8765 端口部署 Waitress 超分后端，一套界面完成生图→超分→矢量化全流程",
      "配置开机自启、计划任务、MWB 远程守护，实现无人值守，4 个服务持续运行无中断",
    ],
    img: "FLUX 工具箱界面截图",
  },
  {
  ...
  },
  {
    ...
    solution: [
      ...
      "tkinter 构建 GUI 界面，PyInstaller 打包为单文件 316MB exe（含完整 OCR 模型），业务员双击即用",
      "跨机器部署至 5090 服务器，解决 DLL 依赖、多页报价单处理、离线模型加载等技术问题",
    ],
    img: "OCR 工具 GUI 界面截图",
  },
  {
    ...
    solution: [
      ...
      "单 HTML 文件架构，无需服务器，微信发给同事打开即用；内置 localStorage 反馈系统，好的 prompt 模板自动沉淀",
    ],
  ...
  },
  {
    ...
    solution: [
      "选用 CIE Lab 色彩空间（非 RGB），Lab 的色差计算与人眼视觉感知一致，聚类结果更自然",
      "K-Means 聚类算法自动将图片颜色归并为指定数量色块，每个色块独立编码色号",
      "输出 SVG 矢量格式：每个颜色区域一个 path，无损缩放，直接对接生产设备",
      "封装为 ComfyUI 自定义节点，用户在生图工作流中即可完成图片→施工模板的全流程",
    ],
    img: "原图 vs 施工模板对比",
  },
  {
  ...
  },
  {
    ...
    solution: [
      ...
      "OneTrainer 框架配置优化（梯度检查点、混合精度、小 batch size），在 8GB 显存极限下完成训练",
